...
class AircraftSimulator(BaseSimulator):
    ...
    def __init__(self,
        ...
        # Connect to DSI
        self.dsi_exec = DSIExec("AcToRl.shm", "RlToDsi.shm", "DsiToRl.shm")
        # initialize simulator
        self.reload()
        #
        self.set_property_value(Catalog.ic_real_joystick_control, 0)
    ...
    def reload(self, new_state: Union[dict, None] = None, new_origin: Union[tuple, None] = None):
        ...
        # Catalog.add_dsi_props(self.dsi_exec.query_property_catalog(""))
        ...
        counter = self.get_property_value(Catalog.ic_get_counter_dsi)
        self.set_property_value(Catalog.ic_set_counter_dsi, counter + 1)
        self.set_property_value(Catalog.ic_reset_dsi, 1)
        # Give the DSI time to set Acceleration correctly
        time.sleep(0.1)
        w = -1
        while  w < counter + 1:
            # wait
            w = self.get_property_value(Catalog.ic_get_counter_dsi)
    ...
    def clear_defalut_condition(self):
        default_condition = {
            Catalog.ic_long_gc_deg: 34.0,  # geodesic longitude [deg]
            Catalog.ic_lat_geod_deg: 32.0,  # geodesic latitude  [deg]
    ...
    def run(self):
        """Runs DSISim simulation until the agent interacts and update custom properties.
        ...
        if self.is_alive:
            if self.bloods <= 0:
                self.shotdown()
            result = self.dsi_exec.run()
            if not result:
                raise RuntimeError("DSISim failed.")
            self._update_properties()
            return result
        else:
            return True
    ...
    def _update_properties(self):
        # update position
        # self._geodetic, self._posture, self._velocity = self.get_ac_state()
        ...
        self._velocity[:] = self.get_property_values([
            ...
            Catalog.velocities_v_west_mps,
            Catalog.velocities_v_up_mps,
        ])
    ...
    def get_property_value(self, prop):
        ...
        if isinstance(prop, Property):
            if prop.access == "R":
                if prop.update:
                    prop.update(self)
            return self.dsi_exec.get_property_value(prop.name_dsisim)
        else:
    ...
    def set_property_value(self, prop, value):
        ...
        if isinstance(prop, Property):
            ...
            self.dsi_exec.set_property_value(prop.name_dsisim, value)
...
# def get_ac_state(self):
    ...
    #         return [[getattr(self.acToRl_sm_data.acData.pos, "lon"),
    #                  getattr(self.acToRl_sm_data.acData.pos, "lat"),
    #                  getattr(self.acToRl_sm_data.acData.pos, "alt")],
    #                 [getattr(self.acToRl_sm_data.acData, "roll"),
    #                  getattr(self.acToRl_sm_data.acData, "pitch"),
    #                  getattr(self.acToRl_sm_data.acData, "yaw")],
    #                 [getattr(self.acToRl_sm_data.vel, "North"),
    #                  getattr(self.acToRl_sm_data.vel, "West"),
    #                  getattr(self.acToRl_sm_data.vel, "Up")]
    #                 ]
    #         # Catalog.position_long_gc_deg,
    #         # Catalog.position_lat_geod_deg,
    #         # Catalog.position_h_sl_m
