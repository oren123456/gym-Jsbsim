...
"""
Property = namedtuple("Property", "name_dsisim description min max access spaces clipped update ")
Property.__new__.__defaults__ = (None, None, float("-inf"), float("+inf"), "RW", Box, True, None)

g = 32.1740  # ft/s^2 standard acceleration due to gravity


class DSICatalog(Property, Enum):
    """
    ...
    position_lat_geod_deg = Property("extra-data.position.lat-geod-deg", "geocentric latitude [deg]", -90, 90, access="R",
                                     update=lambda sim: sim.set_property_value(
                                         DSICatalog.position_lat_geod_deg,
                                         sim.get_property_value(DSICatalog.position_lat_gc_rad) * 57.29))
    position_lat_geod_rad = Property("extra-data.position.lat-geod-rad", "rad", access="R",
                                     update=lambda sim: sim.set_property_value(
    ...
    velocities_eci_velocity_mag_fps = Property("extra-data.velocities.eci-velocity-mag-fps", "fps", access="R",
                                               update=lambda sim: sim.set_property_value(
                                                   DSICatalog.velocities_eci_velocity_mag_fps,
                                                   math.sqrt(
                                                       math.pow(sim.get_property_value(DSICatalog.velocities_v_up_fps), 2) +
                                                       math.pow(sim.get_property_value(DSICatalog.velocities_v_west_fps), 2) +
                                                       math.pow(sim.get_property_value(DSICatalog.velocities_v_north_fps), 2))))
    ...
    velocities_p_rad_sec = Property("extra-data.velocities.p-rad_sec", "roll rate [rad/s]", -2 * math.pi, 2 * math.pi, access="R")
    velocities_q_rad_sec = Property("extra-data.velocities.q-rad_sec", "pitch rate [rad/s]", -2 * math.pi, 2 * math.pi, access="R")
    velocities_r_rad_sec = Property("extra-data.velocities.r-rad_sec", "yaw rate [rad/s]", -2 * math.pi, 2 * math.pi, access="R")
    ...
    accelerations_a_pilot_x_ft_sec2 = Property("ac-rl.ac-data.acc.north", "pilot body x-axis acceleration [ft/sÂ²]", access="R")
    accelerations_a_pilot_y_ft_sec2 = Property("ac-rl.ac-data.acc.west", "pilot body y-axis acceleration [ft/sÂ²]", access="R")
    accelerations_a_pilot_z_ft_sec2 = Property("ac-rl.ac-data.acc.up", "pilot body z-axis acceleration [ft/sÂ²]", access="R")
    accelerations_n_pilot_x_norm = Property("extra-data.accelerations.n-pilot-x-norm", "pilot body x-axis acceleration, normalised", access="R",
                                            update=lambda sim: sim.set_property_value(
                                                DSICatalog.accelerations_n_pilot_x_norm,
                                                sim.get_property_value(DSICatalog.accelerations_a_pilot_x_ft_sec2) / g))
    accelerations_n_pilot_y_norm = Property("extra-data.accelerations.n-pilot-y-norm", "pilot body y-axis acceleration, normalised", access="R",
                                            update=lambda sim: sim.set_property_value(
                                                DSICatalog.accelerations_n_pilot_y_norm,
                                                sim.get_property_value(DSICatalog.accelerations_a_pilot_y_ft_sec2) / g))
    accelerations_n_pilot_z_norm = Property("extra-data.accelerations.n-pilot-z-norm", "pilot body z-axis acceleration, normalised", access="R",
                                            update=lambda sim: sim.set_property_value(
                                                DSICatalog.accelerations_n_pilot_z_norm,
                                                sim.get_property_value(DSICatalog.accelerations_a_pilot_z_ft_sec2) / g))
    ...
    def update_equal_throttle_pos(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_throttle_pos_norm)

    def update_equal_mixture_pos(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_mixture_pos_norm)

    def update_equal_feather_pos(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_feather_pos_norm)

    def update_equal_advance_pos(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_advance_pos_norm)
    ...
    def update_equal_throttle_cmd(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_throttle_cmd_norm)

    def update_equal_mixture_cmd(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_mixture_cmd_norm)

    def update_equal_advance_cmd(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_advance_cmd_norm)

    def update_equal_feather_cmd(sim):
        DSICatalog.update_equal_engine_props(sim, DSICatalog.fcs_feather_cmd_norm)

    @staticmethod
    def update_equal_brake_props(sim):
        value = sim.get_property_value(DSICatalog.fcs_center_brake_cmd_norm)
        sim.jsbsim_exec.set_property_value(DSICatalog.fcs_left_brake_cmd_norm.name_jsbsim, value)
        sim.jsbsim_exec.set_property_value(DSICatalog.fcs_right_brake_cmd_norm.name_jsbsim, value)
    ...
    # dsi reset control
    ic_reset_dsi = Property("rl-dsi.dsi-ctrl-data.reset-scen", "should the dsi reset", 0, 1)
    ic_real_joystick_control = Property("rl-dsi.dsi-ctrl-data.real-joystick-control", "should use joystick", 0, 1)
    ic_set_counter_dsi = Property("rl-dsi.counter", "set dsi counter in RlToDsi shared memory", 0)
    ic_get_counter_dsi = Property("dsi-rl.counter", "set dsi counter in RlToDsi shared memory", 0)
    ...
    ic_psi_true_deg = Property("ic/psi-true-deg", "initial (true) heading [deg]",attitude_psi_deg.min, attitude_psi_deg.max,
                               ...
                               update=lambda sim: sim.set_property_value(
                                   ...
                                   sim.get_property_value(DSICatalog.ic_psi_true_deg) * 0.017453))
    ic_psi_true_rad = Property("rl-dsi.dsi-ctrl-data.start-heading", "rad")
    ...
    ic_roc_fpm = Property("extra-data.ic.roc-fpm", "initial rate of climb [ft/min]")
    ...
    simulation_sim_time_sec = Property("ac-rl.time-tag", "Simulation time [s]", access="R")
...
class ExtraCatalog(Property, Enum):
    ...
    velocities_v_north_mps = Property(
        "extra-data.velocities.v-north-mps", "velocity true north [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_v_north_mps,
            sim.get_property_value(DSICatalog.velocities_v_north_fps) * 0.3048))

    velocities_v_west_mps = Property(
        "extra-data.velocities.v-west-mps", "velocity east [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_v_west_mps,
            sim.get_property_value(DSICatalog.velocities_v_west_fps) * 0.3048))

    velocities_v_up_mps = Property(
        "extra-data.velocities.v-up-mps", "velocity east [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_v_up_mps,
            sim.get_property_value(DSICatalog.velocities_v_up_fps) * 0.3048))

    velocities_v_east_mps = Property(
        "velocities/v-east-mps", "velocity east [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_v_east_mps,
            sim.get_property_value(DSICatalog.velocities_v_east_fps) * 0.3048))
    ...
    velocities_v_down_mps = Property(
        ...
        update=lambda sim: sim.set_property_value(
            ...
            sim.get_property_value(DSICatalog.velocities_v_down_fps) * 0.3048))

    velocities_vc_mps = Property(
        "extra-data.velocities.vc-mps", "airspeed in knots [m/s]", 0, 1400, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_vc_mps,
            math.sqrt(
                math.pow(sim.get_property_value(DSICatalog.velocities_v_up_fps), 2) +
                math.pow(sim.get_property_value(DSICatalog.velocities_v_west_fps), 2) +
                math.pow(sim.get_property_value(DSICatalog.velocities_v_north_fps), 2)) * 0.3048))
    # sim.get_property_value(DSICatalog.velocities_vc_fps) * 0.3048))

    velocities_u_mps = Property(
        "extra-data.velocities.u-mps", "body frame x-axis velocity [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_u_mps,
            # sim.get_property_value(DSICatalog.velocities_u_fps) * 0.3048))
            sim.get_property_value(DSICatalog.velocities_v_north_fps) * 0.3048))

    velocities_v_mps = Property(
        "extra-data.velocities.v-mps", "body frame y-axis velocity [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_v_mps,
            # sim.get_property_value(DSICatalog.velocities_v_fps) * 0.3048))
            sim.get_property_value(DSICatalog.velocities_v_west_fps) * 0.3048))

    velocities_w_mps = Property(
        "extra-data.velocities.w-mps", "body frame z-axis velocity [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
            ExtraCatalog.velocities_w_mps,
            # sim.get_property_value(DSICatalog.velocities_w_fps) * 0.3048))
            sim.get_property_value(DSICatalog.velocities_v_up_fps) * 0.3048))

    def update_delta_altitude(sim):
        value = (sim.get_property_value(ExtraCatalog.target_altitude_ft) - sim.get_property_value(
            DSICatalog.position_h_sl_ft)) * 0.3048
        sim.set_property_value(ExtraCatalog.delta_altitude, value)

    def update_delta_heading(sim):
        value = in_range_deg(
            sim.get_property_value(ExtraCatalog.target_heading_deg) - sim.get_property_value(DSICatalog.attitude_heading_true_rad) * 180 / math.pi)
        sim.set_property_value(ExtraCatalog.delta_heading, value)

    def update_delta_velocities(sim):
        value = (sim.get_property_value(ExtraCatalog.target_velocities_u_mps) - sim.get_property_value(
            ExtraCatalog.velocities_u_mps))
    ...
    def update_throttle_cmd_dir(sim):
        ExtraCatalog.update_property_incr(
            sim, ExtraCatalog.throttle_cmd_dir, DSICatalog.fcs_throttle_cmd_norm, ExtraCatalog.incr_throttle
        )

    def update_aileron_cmd_dir(sim):
        ExtraCatalog.update_property_incr(
            sim, ExtraCatalog.aileron_cmd_dir, DSICatalog.fcs_aileron_cmd_norm, ExtraCatalog.incr_aileron
        )

    def update_elevator_cmd_dir(sim):
        ExtraCatalog.update_property_incr(
            sim, ExtraCatalog.elevator_cmd_dir, DSICatalog.fcs_elevator_cmd_norm, ExtraCatalog.incr_elevator
        )

    def update_rudder_cmd_dir(sim):
        ExtraCatalog.update_property_incr(
            sim, ExtraCatalog.rudder_cmd_dir, DSICatalog.fcs_rudder_cmd_norm, ExtraCatalog.incr_rudder
        )
    ...
    def update_detect_extreme_state(sim):
        ...
        """
        extreme_velocity = sim.get_property_value(DSICatalog.velocities_eci_velocity_mag_fps) >= 1e10
        extreme_rotation = (
                norm(
                    sim.get_property_values(
                        [
                            DSICatalog.velocities_p_rad_sec,
                            DSICatalog.velocities_q_rad_sec,
                            DSICatalog.velocities_r_rad_sec,
                        ]
                    )
                ) >= 1000
        )
        extreme_altitude = sim.get_property_value(DSICatalog.position_h_sl_ft) >= 1e10
        extreme_acceleration = (
                max(
                    [
                        abs(sim.get_property_value(DSICatalog.accelerations_n_pilot_x_norm)),
                        abs(sim.get_property_value(DSICatalog.accelerations_n_pilot_y_norm)),
                        abs(sim.get_property_value(DSICatalog.accelerations_n_pilot_z_norm)),
                    ]
                ) > 1e1
    ...
    delta_altitude = Property(
        "extra-data.position.delta-altitude-to-target-m",
    ...
    delta_heading = Property(
        "extra-data.position.delta-heading-to-target-deg",
    ...
    delta_velocities_u = Property(
        "extra-data.position.delta-velocities_u-to-target-mps",
    ...
    detect_extreme_state = Property(
        "extra-data.detect.extreme-state",
    ...
    target_altitude_ft = Property("extra-data.h-sl-ft","target altitude MSL [ft]",DSICatalog.position_h_sl_ft.min,DSICatalog.position_h_sl_ft.max)
    target_heading_deg = Property("extra-data.target-heading-deg", "target heading [deg]", DSICatalog.attitude_psi_deg.min,
                                  DSICatalog.attitude_psi_deg.max, )
    target_velocities_u_mps = Property("extra-data.tc.target-velocity-u-mps","target heading [mps]",-700,700)
    ...
    heading_check_time = Property("extra-data.heading_check_time", "time to check whether current time reaches heading time", 0)
    overload_check_time = Property("extra-data.overload_check_time", "time to check whether current time reaches overload time", 0)


class MixedCatalog(dict):
    """
    A class to store both dsi & extra properties initiated and used during dsi simulation.
    """

    def __getitem__(self, name):
        try:
            return super().__getitem__(name)
        except KeyError:  # look for the property in ExtraCatalog and DSICatalog
            try:
                self[name] = ExtraCatalog[name].value
            except KeyError:
                self[name] = DSICatalog[name].value
        return super().__getitem__(name)

    def __getattr__(self, name):
        return self[name]

    def add_dsi_props(self, jsbsim_props):
        ...
        for jsbsim_prop in jsbsim_props.splitlines():
