...
class ExtraCatalog(Property, Enum):
    ...
    velocities_v_west_mps = Property(
        "velocities/v-west-mps", "velocity east [m/s]", -700, 700, access="R",
        update=lambda sim: sim.set_property_value(
    ...
    def update_detect_extreme_state(sim):
        ...
        # print(f"{sim.get_property_value(DSICatalog.accelerations_n_pilot_x_norm)}")
        # print(f"{sim.get_property_value(DSICatalog.accelerations_n_pilot_y_norm)}")
        # print(f"{sim.get_property_value(DSICatalog.accelerations_n_pilot_z_norm)}")
    ...
    delta_altitude = Property(
        "position/delta-altitude-to-target-m", "delta altitude to target [m]", -40000, 40000, access="R",  update=update_delta_altitude, )
    delta_heading = Property(
        "position/delta-heading-to-target-deg", "delta heading to target [deg]", -180, 180, access="R", update=update_delta_heading)
    delta_velocities_u = Property(
        "position/delta-velocities_u-to-target-mps", "delta velocities_u to target", -1400, 1400, access="R", update=update_delta_velocities, )
    # controls command
    ...
    detect_extreme_state = Property(
        "detect/extreme-state", "detect extreme rotation, velocity and altitude", 0, 1, spaces=Discrete, access="R", update=update_detect_extreme_state, )
    # target conditions
    target_altitude_ft = Property("tc/h-sl-ft", "target altitude MSL [ft]", DSICatalog.position_h_sl_ft.min, DSICatalog.position_h_sl_ft.max)
    target_heading_deg = Property("tc/target-heading-deg", "target heading [deg]", DSICatalog.attitude_psi_deg.min,
                                  DSICatalog.attitude_psi_deg.max, )
    target_velocities_u_mps = Property("tc/target-velocity-u-mps", "target heading [mps]", -700, 700)
    ...
    heading_check_time = Property("heading_check_time", "time to check whether current time reaches heading time", 0)
